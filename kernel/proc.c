...
found:
  ...
  if(pa == 0)
    panic("kalloc");
  uint64 va = KSTACK(0);
  kvmmapforproc(p->kernel_pagetable, va, (uint64)pa, PGSIZE, PTE_R | PTE_W);
  p->kstack = va;
...
freeproc(struct proc *p)
{
  ...
  freewalklevel(p->kernel_pagetable, 2);
...
growproc(int n)
{
  ...
  if(n > 0){
    ...
    kuvmdealloc(p->kernel_pagetable, p->sz, p->sz +n);
  }
  p->sz = sz;
  return 0;
}
...
fork(void)
{
  ...
  // Copy pagetable to kernel pagetable.
